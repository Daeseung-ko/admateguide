...
(function () {
  ...
  M.adGroup = function () {
    return secDetails() + secDemographics() + secDelivery() + secAdvTargeting();
  };
  ...
  function secDelivery() {
    ...
    return sectionCard('delivery', '⚡', '전달 및 게재 위치', `
      <div class="sim-row">
        <div class="sim-field">
          ${flabel('최적화 목표')}
          <input class="sim-input" value="${obj.goal}" readonly style="background:#fafaf9;color:#8c8880">
        </div>
        <div class="sim-field">
          ${flabel('결제 방법')}
          ${state.bid === 'max' ? `
          <!-- 최대 입찰가는 동영상 조회당 과금만 가능 → CPV 고정 -->
          <select class="sim-input sim-select" disabled style="background:#fafaf9;color:#8c8880">
            <option selected>동영상 조회수 (CPV)</option>
          </select>` : `
          <select class="sim-input sim-select" onchange="SIM.setSeg('payMethod', this.value)">
            <option value="cpm" ${state.payMethod !== 'cpv' ? 'selected' : ''}>노출수 (CPM)</option>
            <option value="cpv" ${state.payMethod === 'cpv' ? 'selected' : ''}>동영상 조회수 (CPV)</option>
          </select>`}
        </div>
      </div>
      <div class="sim-field">
        ...
        ${radios('bid', [
          ...
          { v: 'max',  label: '최대 입찰가', desc: '지불할 최대 금액을 설정하세요. 높게 입찰할수록 광고가 더 자주 표시됩니다.' },
        ])}
        ${state.bid === 'max' ? `
          <div style="margin-top:12px">
            <div class="sim-krw">
              <span class="sim-krw-cur">₩</span>
              <input type="text" inputmode="numeric" placeholder="0.00"
                     value="${state.maxBid === '' ? '' : (+state.maxBid).toLocaleString()}"
                     oninput="SIM.onMoney('maxBid', this)">
              <span class="sim-krw-suffix">동영상 조회당</span>
            </div>
            <div class="sim-fdesc" style="margin:7px 0 0">동영상 조회당 지불할 의사가 있는 최대 금액을 설정하세요.</div>
          </div>` : ''}
      </div>
      <div class="sim-field">
        ${flabel('게재 위치')}
