/* ══════════════════════════════════════════════
   참여수(engagement) 캠페인 — 광고그룹 단계
   core.js 의 공용 헬퍼·상태를 사용한다.
   이 파일만 수정하면 이 목표의 광고그룹 화면이 바뀐다.
══════════════════════════════════════════════ */
(function () {
  window.CAMPAIGN_MODULES = window.CAMPAIGN_MODULES || {};
  const M = (window.CAMPAIGN_MODULES.engagement = window.CAMPAIGN_MODULES.engagement || {});
  ...
  function secDelivery() {
    ...
    return sectionCard('delivery', '⚡', '전달 및 게재 위치', `
      <div class="sim-row">
        ...
        <div class="sim-field">
          ${flabel('결제 방법')}
          <input class="sim-input" value="${obj.pay}" readonly style="background:#fafaf9;color:#8c8880">
        </div>
      </div>
      <div class="sim-field">
        ${flabel('입찰 전략')}
        ...
        ${state.bid === 'max' ? `
          <div style="margin-top:12px">
            <div class="sim-krw">
              ...
              <span class="sim-krw-suffix">참여당</span>
            </div>
            <div class="sim-fdesc" style="margin:7px 0 0">참여당 지불할 의사가 있는 최대 금액을 설정하세요.</div>
          </div>` : ''}
      </div>
      <div class="sim-field">
        ${flabel('게재 위치')}
