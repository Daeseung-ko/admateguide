    ...
    return sectionCard('delivery', '⚡', '전달 및 게재 위치', `
      <div class="sim-row">
        ...
        <div class="sim-field">
          ${flabel('결제 방법')}
          ${state.bid === 'max' ? `
          <!-- 최대 입찰가는 링크 클릭당 과금만 가능 → CPC 고정 -->
          <select class="sim-input sim-select" disabled style="background:#fafaf9;color:#8c8880">
            <option selected>링크 클릭수 (CPC)</option>
          </select>` : `
          <select class="sim-input sim-select" onchange="SIM.setSeg('payMethod', this.value)">
            <option value="cpm" ${state.payMethod !== 'cpc' ? 'selected' : ''}>노출수 (CPM)</option>
            <option value="cpc" ${state.payMethod === 'cpc' ? 'selected' : ''}>링크 클릭수 (CPC)</option>
          </select>`}
        </div>
      </div>
      <div class="sim-field">
        ...
        ${state.bid === 'max' ? `
          <div style="margin-top:12px">
            <div class="sim-krw">
              ...
              <span class="sim-krw-suffix">링크 클릭당</span>
            </div>
            <div class="sim-fdesc" style="margin:7px 0 0">링크 클릭당 지불할 의사가 있는 최대 금액을 설정하세요.</div>
          </div>` : ''}
      </div>
      <div class="sim-field">
        ${flabel('게재 위치')}
